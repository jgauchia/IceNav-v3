...
typedef struct 
{
    const char *name;       /**< Human-readable time zone name */
    const char *posix_str;  /**< POSIX time zone string representation */
} posix_tz;
...
{
    int arraySize = sizeof(posix_tz_db) / sizeof(posix_tz_db[0]);

    for (int i = 0; i < arraySize; i++) 
    {
        if (strcmp(posix_tz_db[i].name, name) == 0)  
            return posix_tz_db[i].posix_str;
    }
    return "UTC";
}